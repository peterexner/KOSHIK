...
public class LTHSemanticRoleLabeler implements ContentProcessor {
	...
	private int maxSentenceTokens;
	
	public LTHSemanticRoleLabeler(String modelFileName, L l, int maxSentenceTokens) {
		try {
		...
		this.maxSentenceTokens = maxSentenceTokens;
	}
	...
	public void process(Document document) throws Exception {
		for(se.lth.cs.koshik.model.text.Sentence sentence:document.select(se.lth.cs.koshik.model.text.Sentence.class)) {
			...
			if(tokens.length > this.maxSentenceTokens) {
				continue;
			}
			...
			Sentence parsedLTHSentence = new Sentence(sentenceData09);
