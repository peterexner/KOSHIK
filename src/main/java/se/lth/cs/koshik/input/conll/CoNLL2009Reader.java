	...
	private void buildDependencyEdges(List<String> sentenceLines, Document document, Sentence sentence) {
		...
		for(String sentenceLine:sentenceLines) {
			...
			if(!fields[9].equalsIgnoreCase("_") && !fields[11].equalsIgnoreCase("_")) {
				sentence.getToken(index).setFeature(CoNLLFeature.HEAD, fields[9]);
				sentence.getToken(index).setFeature(CoNLLFeature.DEPREL, fields[11]);	
			}
		...
		for(String sentenceLine:sentenceLines) {
			...
			for(Token predicateToken:sentence.getPredicateTokens()) {
				if(!fields[14+i].equalsIgnoreCase("_")) {
					sentence.getToken(index).setFeature(CoNLLFeature.APRED, sentence.getToken(index).getFeature(CoNLLFeature.APRED) + ";" + predicateToken.getFeature(CoNLLFeature.ID) + ":" + fields[14+i]);
				}
				i++;
			}
		}
