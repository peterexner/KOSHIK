...
public class ReadDocument extends Configured implements Tool {
	private static final String OPTION_INPUTPATHS = "input";
	private static final String OPTION_OUTPUTPATH = "output";


	private static final Logger LOGGER = Logger.getLogger(ReadDocument.class);
			...
			try {
				for(Sentence sentence:document.select(Sentence.class)) {
					...
					for(Token token:sentence.getTokens()) {
						sb.append('\t');
						sb.append('\t');
						sb.append(token.getBegin());
						sb.append('-');
						sb.append(token.getEnd());
						sb.append(':');
						sb.append('\t');
						...
						if(token.hasFeature(CoNLLFeature.APRED)) {
							sb.append('\t');
							sb.append(token.getFeature(CoNLLFeature.APRED));
						} else {
							sb.append('\t');
							sb.append('_');
						}
	...
	public static void main(String[] args) throws Exception {
		int exitCode = ToolRunner.run(new ReadDocument(), args);
